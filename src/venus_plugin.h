...
// Should follow the exact same order as the array declared in main.c, altered - array now in
// contract.c
typedef enum {
...
typedef enum {
    ...
    // *** Comptroller
    COLLATERAL_TOKENS_OFFSET,  // start of parameters for arrayof vTokens for
                               // VENUS_PROVIDE_COLLATERAL
    ...
    // CLAIM_XVS_HOLDER address, CLAIM_XVS_vTOKENS array used for CLAIM_VENUS functions - Removed
    // from Spec

    // *** Vaults
    DEPOSIT_AMOUNT,  // uint256 single parameter for VAULT_DEPOSIT

    REWARD_TOKEN,         // address for VAULT_DEPOSIT_TOKEN, VAULT_WITHDRAW_TOKEN_REQUEST and
                          // VAULT_WITHDRAW_EXECUTE
    VAULT_PID,            // uint256
    ...
    // VAULT_NAME,             // VAULT_CLAIM - has no parameters for VRT and VAI. For XVS claim
    // call VAULT_DEPOSIT_TOKEN with zero 0 amount
    ...
    // CLAIM_XVS,               // VENUS_WITHDRAW_VESTED_XVS - has no parameters just displays
    // "Claim XVS"
...
typedef struct context_t {
    ...
    /* Not enough room for more specific selectors, must map specifc fields to the one above as
    shown uint8_t dest[ADDRESS_LENGTH]; use beneficiary[ADDRESS_LENGTH] uint8_t
    collateral[ADDRESS_LENGTH]; use contract_address_sent[ADDRESS_LENGTH] uint8_t
    amount[INT256_LENGTH]; use amount_sent[INT256_LENGTH] uint8_t proposal_id[INT256_LENGTH]; use
    amount_sent[INT256_LENGTH] uint8_t support[INT256_LENGTH];   use amount_received[INT256_LENGTH]
